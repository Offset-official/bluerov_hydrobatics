...
def load_trajectory_from_csv(file_path):
    ...
    with open(file_path, 'r') as f:
        ...
        if header != ['x', 'y', 'z']:
            print(f"Warning: CSV header {header} doesn't match expected format ['x', 'y', 'z']")
...
class TrajectoryVisualizer:
    ...
    def __init__(self):
        ...
        self.vis.open()
        print("Meshcat server started. Open the URL below in your browser if it doesn't open automatically.")
            ...
            for i, point in enumerate(points):
                ...
                if i == 0:
                    ...
                    self.vis["markers/start"].set_object(
                        g.Sphere(0.2),
                        g.MeshPhongMaterial(color=0x00FF00)
                    )
                    ...
                    self.vis["markers/end"].set_object(
                        g.Sphere(0.2),
                        g.MeshPhongMaterial(color=0xFF0000)
                    )
                    self.vis["markers/end"].set_transform(
                        tf.translation_matrix(point)
                    )
                else:
...
def main():
    parser = argparse.ArgumentParser(description="Visualize a 3D trajectory with BlueROV2 model")
    parser.add_argument('--file', type=str, required=True, 
                        help='Path to CSV file with trajectory waypoints')
    parser.add_argument('--loop', action='store_true', 
                        help='Loop the trajectory animation continuously')
    parser.add_argument('--speed', type=float, default=1.0,
                        help='Animation speed factor (default: 1.0)')
